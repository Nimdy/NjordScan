...
class ReportFormatter:
    ...
    def save_markdown_report(self, results: Dict[str, Any], output_file: str):
...
<body>
    ...
    def _save_markdown_report(self, results: Dict[str, Any], output_path: Path):
        """Save report as Markdown format."""
        from datetime import datetime
    ...
    def _generate_markdown_content(self, results: Dict[str, Any]) -> str:
        ...
        # Header
        content = f"""# 🛡️ NjordScan Security Report

**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  
**Target:** `{results.get('target', 'Unknown')}`  
**Framework:** `{results.get('framework', 'Unknown')}`  
**Scan Mode:** `{results.get('scan_mode', 'Unknown')}`  
**Duration:** {results.get('scan_duration', 0):.2f} seconds
...
{self._generate_markdown_vulnerabilities(results)}

---

## 📈 Statistics

{self._generate_markdown_statistics(results)}
...
{self._generate_markdown_scan_info(results)}
...
"""
    ...
    def _generate_markdown_summary(self, results: Dict[str, Any]) -> str:
        ...
        score = njord_score.get('score', 0)
        grade = njord_score.get('grade', 'N/A')
...
- ℹ️ Info: {severity_breakdown.get('info', 0)}

**Recommendation:** {njord_score.get('recommendation', 'No specific recommendation available')}
"""
        return content
    
    def _generate_markdown_vulnerabilities(self, results: Dict[str, Any]) -> str:
        ...
        for module, vulns in vulnerabilities.items():
            ...
            for vuln in vulns:
                if isinstance(vuln, dict):
                    ...
                    line_number = vuln.get('line_number', 'Unknown line')
                    ...
                    content += f"""
**{severity_emoji} {severity.upper()}** - {title}
...
"""
    ...
    def _generate_markdown_statistics(self, results: Dict[str, Any]) -> str:
        """Generate markdown statistics section."""
        modules_run = results.get('modules_run', [])
        plugins_run = results.get('plugins_run', [])
        scan_duration = results.get('scan_duration', 0)
        
        content = f"""
**Modules Executed:** {', '.join(modules_run)}

**Plugins Executed:** {', '.join(plugins_run) if plugins_run else 'None'}

**Scan Duration:** {scan_duration:.2f} seconds

**AI Enhanced:** {'Yes' if results.get('ai_enhanced', False) else 'No'}
"""
        return content
    ...
    def _generate_markdown_recommendations(self, results: Dict[str, Any]) -> str:
        ...
        njord_score = results.get('njord_score', {})
        next_steps = njord_score.get('next_steps', [])
    ...
    def _generate_markdown_scan_info(self, results: Dict[str, Any]) -> str:
        """Generate markdown scan information section."""
        content = f"""
**Target:** `{results.get('target', 'Unknown')}`
...
**Timestamp:** {results.get('scan_duration', 0):.2f} seconds
