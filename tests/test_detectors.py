...
async def test_secrets_detects_env_file_credentials(vuln_app):
    ...
    # V1 missed all of these in a .env file. (Provider-format keys — AWS/Stripe/GitHub —
    # are covered by the runtime-built tests below so the fixture holds no committable
    # real-format secret that would trip GitHub push protection.)
    assert "secret.generic" in ids            # db url with credentials
...
async def test_secrets_detects_aws_key(tmp_path):
    """AWS-key coverage, built at runtime so no committable `AKIA…` literal exists in
    the repo (which would trip GitHub push protection / secret scanners)."""
    aws = "AK" + "IA" + "5XQP7TZR9WMV2KLN"  # synthetic, not a real key
    (tmp_path / "package.json").write_text('{"name":"t"}')
    (tmp_path / ".env").write_text(f"AWS_ACCESS_KEY_ID={aws}\n")
    result = await scan(tmp_path, only_detectors=["secrets"])
    assert "secret.aws-access-key" in rule_ids(result.findings)
